...
import { Loader2, Eye, Calendar, ChevronLeft, ChevronRight, CheckCircle2, XCircle } from 'lucide-react'
import {
  Tooltip,
  TooltipContent,
  TooltipProvider,
...
interface PendingTask { invoiceId: number; taskId: string }
...
function removePendingTask(invoiceId: number) {
  try {
    const tasks: PendingTask[] = JSON.parse(localStorage.getItem(PENDING_TASKS_KEY) || '[]')
    localStorage.setItem(PENDING_TASKS_KEY, JSON.stringify(tasks.filter(t => t.invoiceId !== invoiceId)))
  } catch {}
}
      ...
      <DialogContent className="!max-w-5xl !w-[95vw] !max-h-[92vh] p-6 overflow-y-auto">
        <DialogHeader>
          ...
          <DialogDescription className="text-base text-gray-600 space-y-1">
            <div><span className="font-semibold">Data:</span> {new Date(invoice.DataFactura).toLocaleDateString('pt-PT')}</div>
            <div><span className="font-semibold">Referência Doc*:</span> {invoice.Referencia || '—'}</div>
            {invoice.Descricao && (
        ...
        <div className="space-y-6 mt-4">
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
            <div><p className="font-semibold">Valor Total</p><p className="text-xl font-bold text-primary">{new Intl.NumberFormat('pt-PT', { style: 'currency', currency: 'AOA' }).format(invoice.TotalPreco)}</p></div>
            <div><p className="font-semibold">Itens</p><p className="text-xl font-bold">{invoice.itens.length}</p></div>
            <div><p className="font-semibold">Multas</p><p className="text-xl font-bold text-orange-600">{new Intl.NumberFormat('pt-PT', { style: 'currency', currency: 'AOA' }).format(totalMultas)}</p></div>
            <div><p className="font-semibold">Pago</p><p className="text-xl font-bold text-green-600">{new Intl.NumberFormat('pt-PT', { style: 'currency', currency: 'AOA' }).format(totalPago)}</p></div>
          </div>
              ...
              {invoice.itens.map((item, idx) => (
                <Card key={item.codigo} className="p-4">
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-sm">
                    <div><p className="font-medium">{`${idx + 1} -`} {item.OBS || item.DescricaoServico || 'Sem descrição'}</p></div>
                    <div className="space-y-1">
                      <div className="flex justify-between"><span>Preço Unitário:</span><span className="font-mono">{new Intl.NumberFormat('pt-PT', { style: 'currency', currency: 'AOA' }).format(item.preco)}</span></div>
                      <div className="flex justify-between"><span>Quantidade:</span><span className="font-mono">{item.Quantidade}</span></div>
                      <div className="flex justify-between"><span>Subtotal:</span><span className="font-mono font-semibold">{new Intl.NumberFormat('pt-PT', { style: 'currency', currency: 'AOA' }).format(item.Total)}</span></div>
                    </div>
                    <div className="space-y-1 text-xs">
                      {item.taxa_iva > 0 && <div className="flex justify-between"><span>IVA ({item.taxa_iva}%):</span><span className="font-mono">{new Intl.NumberFormat('pt-PT', { style: 'currency', currency: 'AOA' }).format(item.valor_iva)}</span></div>}
                      {item.Multa > 0 && <div className="flex justify-between text-orange-600"><span>Multa:</span><span className="font-mono">{new Intl.NumberFormat('pt-PT', { style: 'currency', currency: 'AOA' }).format(item.Multa)}</span></div>}
                      {item.valor_desconto > 0 && <div className="flex justify-between text-green-600"><span>Desconto:</span><span className="font-mono">-{new Intl.NumberFormat('pt-PT', { style: 'currency', currency: 'AOA' }).format(item.valor_desconto)}</span></div>}
                      {item.valor_pago > 0 && <div className="flex justify-between text-green-700 font-medium"><span>Pago:</span><span className="font-mono">{new Intl.NumberFormat('pt-PT', { style: 'currency', currency: 'AOA' }).format(item.valor_pago)}</span></div>}
                    </div>
                  </div>
                </Card>
              ))}
          ...
          <div className="flex gap-3 pt-4 border-t">
            {invoice.estado === 1 ? (
              <PaymentReceipt invoice={invoice} academicYear={findAcademicYearDesignation(invoice.ano_lectivo)} />
            ) : (
              <PaymentReceipt2 invoice={invoice} academicYear={findAcademicYearDesignation(invoice.ano_lectivo)} />
            )}
          </div>
        </div>
      </DialogContent>
...
export function InvoicesTable({
  ...
  const { data, isLoading, isError } = useQueryInvoices({
    ...
    status: statusFilter === 'all' ? undefined : statusFilter === 'paid' ? 1 : 0,
  })
  ...
  if (isLoading || isError || isLoadingAcademicYear) return <InvoicesTableSkeleton />
    ...
    <Card className="overflow-hidden shadow-sm">
      <CardHeader className="border-b bg-muted/30">
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          <CardTitle className="text-2xl font-bold">Nota de Pagamentos</CardTitle>
  ...
  const columns: ColumnDef<Invoice>[] = [
    ...
    { accessorKey: 'Referencia', header: 'Número Doc*' },
    {
      ...
      cell: ({ row }) => {
        ...
        const ref = refs.filter(r => r.Status === 'Pending').sort((a, b) => new Date(b.END_DATE).getTime() - new Date(a.END_DATE).getTime())[0] || refs[0]
              ...
              <AlertDialog>
                ...
                <AlertDialogContent>
                  <AlertDialogHeader>
                    ...
                    <AlertDialogDescription>
                      Esta ação irá gerar uma nova referência Multicaixa/MB Way para a Nota de Pagamento{' '}
                  ...
                  <AlertDialogFooter>
                    ...
                    <AlertDialogAction
                      onClick={() => gerarRefMutation.mutate({ codigoFactura: invoice.Codigo })}
                      disabled={gerarRefMutation.isPending || isPolling}
                    >
                      {gerarRefMutation.isPending || isPolling ? (
                        <>
