import { CalendarClock, CheckCircle2, Circle, Clock, GraduationCap, Mail, MapPin } from 'lucide-react'
...
export function RegistrationsDeadlineExpired({
  dataLimite,
  proximoPeriodo,
  aindaNaoComecou = false,
}: RegistrationsDeadlineExpiredProps) {
  const timelineSteps = aindaNaoComecou
    ? [
        { label: 'Matrícula', status: 'done' as const },
        { label: 'Período atual', status: 'current' as const },
        { label: 'Confirmação de disciplinas', status: 'upcoming' as const },
    ...
    <Card className="animate-in fade-in-0 zoom-in-95 slide-in-from-bottom-2 overflow-hidden border-primary/20 bg-gradient-to-br from-primary/5 via-primary/[0.03] to-transparent duration-500">
      <CardContent className="flex flex-col items-center px-6 py-12 text-center">
        ...
        <h3 className="mb-2 text-xl font-bold">
          {aindaNaoComecou
            ? 'A confirmação ainda não abriu'
            : 'Acompanha o teu percurso'}
        </h3>

        <p className="max-w-md text-sm leading-relaxed text-muted-foreground">
          {aindaNaoComecou
            ? proximoPeriodo
              ? `A confirmação de disciplinas abre em ${proximoPeriodo}. Fica atento para não perderes o prazo.`
              : 'A confirmação de disciplinas ainda não abriu. Fica atento para não perderes o prazo.'
            : dataLimite
          ...
          <div className="flex items-start gap-3 rounded-lg border bg-background/60 p-4 text-left">
            ...
            <div>
              <p className="text-sm font-medium">
                {aindaNaoComecou ? 'Abre em' : 'Próximo período'}
              </p>
