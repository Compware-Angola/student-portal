...
export const MensagensNotificacoes = () => {
 // const [selectedYear, setSelectedYear] = useState("2024-2025")
  ...
  const { data: mensagens, isLoading: isLoadingMensagens } = useQueryMessage({ userId})

  // ✅ Busca COMUNICADOS
  const { data: comunicados, isLoading: isLoadingComunicados } = useQueryAnnouncement({ pre_inscricao})
   ...
   */}
      ...
      <Tabs defaultValue="mensagens" className="w-full">
        <TabsList className="grid w-full grid-cols-2">
          <TabsTrigger value="mensagens">Mensagens ({normalizedMensagens.length})</TabsTrigger>
          <TabsTrigger value="comunicados">Comunicados ({normalizedComunicados.length})</TabsTrigger>
        </TabsList>

        {/* Aba de Mensagens */}
        <TabsContent value="mensagens" className="space-y-4 mt-6">
          {normalizedMensagens.length === 0 ? (
            <p className="text-center text-muted-foreground">Nenhuma mensagem disponível.</p>
          ) : (
            normalizedMensagens
              .sort((a, b) => new Date(b.date).getTime() - new Date(a.date).getTime())
              .map((msg) => (
                <Card key={msg.id} className={!msg.read ? "border-l-4 border-l-primary" : ""}>
                  <CardHeader>
                    <div className="flex items-start justify-between">
                      ...
                      <Badge variant={getPriorityColor(msg.priority)}>Normal</Badge>
                    </div>
                  </CardHeader>
                    ...
                    <div className="space-y-3 pl-4 border-l-2 border-primary">
                      {msg.respostas ? (
                        <div className="bg-muted p-3 rounded-md">
                          ...
                          <p className="text-sm text-muted-foreground">{msg.respostas.mensagem_resposta ?? "Sem resposta"}</p>

                          {msg.respostas.data_resposta && (
                            <p className="text-[12px] text-muted-foreground mt-2">
                              {new Date(msg.respostas.data_resposta).toLocaleDateString("pt-PT", {
                                day: "2-digit",
                                month: "long",
                                year: "numeric",
                                hour: "2-digit",
        ...
        <TabsContent value="comunicados" className="space-y-4 mt-6">
          {normalizedComunicados.length === 0 ? (
            <p className="text-center text-muted-foreground">Nenhum comunicado disponível.</p>
          ) : (
                ...
                <Card key={com.id}>
                  <CardHeader>
                    <div className="flex items-start justify-between">
                      ...
                      <Badge variant={getPriorityColor(com.priority)}>Baixa</Badge>
                    </div>
                  </CardHeader>
                  <CardContent>
