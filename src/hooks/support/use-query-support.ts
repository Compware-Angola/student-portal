import { createSupport, getAllSupportTypes, type RespostaMensagens, type SupportPayload, type TipoSuporte } from "@/services/support/support.service"
...
export function useQueryCreateSupport() {
  ...
  const mutation = useMutation<RespostaMensagens, any, SupportPayload>({
    ...
    onSuccess: (data) => {
      if (data.success) {
        toast('O seu pedido de suporte foi enviado com sucesso.')
    ...
    onError: (error: any) => {
      toast.error(error?.message || 'Ocorreu um erro ao enviar o suporte. Tente novamente.')
    },
  })
